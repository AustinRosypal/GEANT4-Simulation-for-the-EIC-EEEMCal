...
def select_all_fiducial(total, region, r):
    """
    Apply the same fiducial radial cuts used for the two materials,
...
for fname in files:
    ...
    # All events now use the same fiducial region cuts as the two material samples
    ...
    # Crystal Ball fit-based resolutions on the original fiducial subsets
    # AUSTIN ADDED THE BELOW THREE LINES
...
# -----------------------------
# Plot 1: Crystal Ball resolutions, no error bars
# -----------------------------
...
# Bootstrap settings for error bars
NBOOT = 200
...
def extract_energy_from_filename(fname):
    ...
    Match filenames like:
      ...
      ecal_10GeV_t5.root
    """
    m = re.search(r'ecal_(\d+(?:\.\d+)?)GeV_.*Apr22.*\.root', fname)
    if not m:
        return None
    return float(m.group(1))
...
def rms_resolution(arr):
    """
    Compute RMS/mean resolution:
        mu = mean(arr)
        sigma = sample std(arr)
        resolution = sigma / mu
    """
    arr = np.asarray(arr)
    arr = arr[np.isfinite(arr)]
...
def crystal_ball_left(x, A, mu, sigma, alpha, n):
    """
    Left-tailed Crystal Ball function.

    Gaussian core for x near mu, with a power-law tail on the low-energy side.
    Parameters:
        A     : normalization
        mu    : peak position
        sigma : Gaussian-core width
        alpha : transition point in sigma units (>0)
        n     : tail exponent (>1)
    """
    x = np.asarray(x)
    sigma = np.abs(sigma)
...
def crystal_ball_resolution(arr, nbins=None):
    """
    Fit a left-tailed Crystal Ball function to the histogram of arr.

    Returns:
        mu_fit, sigma_fit, resolution, xcurve, ycurve, counts, edges

    The plotted/quoted resolution is:
        resolution = sigma_fit / mu_fit
    where sigma_fit is the Gaussian-core width parameter of the
    ...
    # Wider fit region on the low-energy side to accommodate the tail.
    mask = (centers > 0.60 * mu0) & (centers < 1.08 * mu0)
    xfit = centers[mask]
    yfit = counts[mask]

    # Only fit populated bins.
    populated = yfit > 0
    xfit = xfit[populated]
    yfit = yfit[populated]
...
def crystal_ball_resolution_wrapper(arr):
    """
    Wrapper for bootstrap error estimation.
    """
    mu, sigma, res, _, _, _, _ = crystal_ball_resolution(arr)
    return mu, sigma, res


def bootstrap_resolution_errors(arr, resolution_func, nboot=NBOOT, rng=RNG):
    """
    Estimate uncertainty on the resolution by bootstrap resampling.
    """
    arr = np.asarray(arr)
    arr = arr[np.isfinite(arr)]
...
# Find files
# -----------------------------
all_files = glob.glob("ecal_*GeV_Apr22*.root")
files = [f for f in all_files if extract_energy_from_filename(f) is not None]
files = sorted(files, key=lambda f: extract_energy_from_filename(f))
...
for fname in files:
    ...
    # RMS-based resolutions
    _, _, rall_rms = rms_resolution(all_events)
    _, _, rpwo_rms = rms_resolution(pwo_events)
    _, _, rsci_rms = rms_resolution(sci_events)
    ...
    # Bootstrap errors on Crystal Ball resolutions
    err_all = bootstrap_resolution_errors(all_events, crystal_ball_resolution_wrapper)
    err_pwo = bootstrap_resolution_errors(pwo_events, crystal_ball_resolution_wrapper)
    err_sci = bootstrap_resolution_errors(sci_events, crystal_ball_resolution_wrapper)
    ...
    err_all_cb.append(err_all)
    err_pwo_cb.append(err_pwo)
    err_sci_cb.append(err_sci)

    # Print summary for this file
    print(fname)
    print_stats("All events:", all_events, rall_rms, rall_cb)
    print_stats("PWO fiducial:", pwo_events, rpwo_rms, rpwo_cb)
    print_stats("SciGlass fiducial:", sci_events, rsci_rms, rsci_cb)
...
err_all_cb = np.array(err_all_cb, dtype=float)
err_pwo_cb = np.array(err_pwo_cb, dtype=float)
err_sci_cb = np.array(err_sci_cb, dtype=float)

print("\nenergies =", energies)
print("res_all_cb =", res_all_cb)
print("res_pwo_cb =", res_pwo_cb)
print("res_sci_cb =", res_sci_cb)
print("err_all_cb =", err_all_cb)
print("err_pwo_cb =", err_pwo_cb)
print("err_sci_cb =", err_sci_cb)

# -----------------------------
# Plot 1: Crystal Ball resolutions with error bars
# -----------------------------
plt.figure(figsize=(9, 6))
...
plt.errorbar(
    energies,
    finite_percent(res_pwo_cb),
    yerr=finite_percent(err_pwo_cb),
    fmt='o-',
    capsize=4,
    label='PbWO4 (Crystal Ball)'
)

plt.errorbar(
    energies,
    finite_percent(res_sci_cb),
    yerr=finite_percent(err_sci_cb),
    fmt='s-',
    capsize=4,
    label='SciGlass (Crystal Ball)'
)

plt.errorbar(
    energies,
    finite_percent(res_all_cb),
    yerr=finite_percent(err_all_cb),
    fmt='^-',
    capsize=4,
    label='All Events (Crystal Ball)'
)

plt.xlabel("Beam Energy (GeV)")
plt.ylabel("Energy Resolution (%)")
plt.title("EEEMCal Resolution vs. Energy (Crystal Ball Fit)")
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
# -----------------------------
# Plot 2: RMS/mean resolutions for comparison
# -----------------------------
plt.figure(figsize=(9, 6))

plt.plot(energies, finite_percent(res_pwo_rms), 'o-', label='PbWO4 (RMS/mean)')
plt.plot(energies, finite_percent(res_sci_rms), 's-', label='SciGlass (RMS/mean)')
plt.plot(energies, finite_percent(res_all_rms), '^-', label='All Events (RMS/mean)')

plt.xlabel("Beam Energy (GeV)")
plt.ylabel("Energy Resolution (%)")
plt.title("EEEMCal Resolution vs. Energy (RMS/mean)")
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
