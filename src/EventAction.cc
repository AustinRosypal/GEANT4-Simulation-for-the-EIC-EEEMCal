...
void EventAction::EndOfEventAction(const G4Event* event)
{
    ...
    for (G4int i = 0; i < nHits; ++i) {
        fTotalEdep += (*hitsCollection)[i]->GetEdep();
    }
    // G4cout << "Edep: " << fTotalEdep/MeV << " MeV" << G4endl;
    if (event->GetEventID() % 100 == 0)
      G4cout << "Finished event " << event->GetEventID() << G4endl;

    auto* analysisManager = G4AnalysisManager::Instance();
    analysisManager->FillH1(0, fTotalEdep / MeV);

    analysisManager->FillNtupleDColumn(0, fTotalEdep / MeV);
    analysisManager->FillNtupleIColumn(1, fNHits);
    analysisManager->AddNtupleRow();
} */
